...
| 3-2 | 완료(배포 전) | 2026-09-21. supabase/functions/identify: PlantNet 연결. 순수 로직은 _shared/plantnet.ts(응답에서 후보 3개, 점수 0.01 미만은 버림), 한도는 _shared/limits.ts 와 bump_usage 함수(마이그레이션 20260921010000, 동시 요청에도 정확히 세고 48시간 지난 기록은 지움). 한도값은 coefficients 에서 읽는다. 사진은 넘기는 동안 메모리에만 둔다. 테스트 14개 |
| 3-3 | 완료(배포 전) | 2026-09-21. supabase/functions/species: 종 DB 에 없으면 Claude 로 만들어 검증 후 저장. _shared/species-schema.ts 는 zod 스키마와 상식 체크(다육인데 주기가 짧음, 열대인데 실내 월동 불가, 수종군·식물군 어긋남 등), 거부되면 이유를 넣어 한 번만 재생성하고 두 번 다 실패하면 pending. 모델은 claude-sonnet-5(SPEC 9.2 의 sonnet-4-6 보다 뒤 세대). 테스트 17개 |
