...
| 3-4 | 완료(실행 전) | 2026-09-21. scripts/species-list.ts 300종(SPEC 10.5 구성대로, 번들 30종과 철자 일치, 학명·국명 중복 없음). scripts/seed-species.ts 는 배포된 species 함수를 학명마다 부른다. 프롬프트와 검증이 갈라지지 않고, 이미 있는 종은 건너뛰어 끊겨도 다시 돌리면 된다. pnpm seed:species [--limit N] [--group X]. 실행은 함수 배포 뒤 |
| 3-5 | 완료(배포 전) | 2026-09-21. 고른 종은 출처와 무관하게 KnownSpecies 로 같게 다룬다(registration.ts). plants.base_interval 추가(0003)로 엔진이 시드 목록에 기대지 않고 오프라인에서도 같은 값으로 센다. api/client.ts(설정 없으면 호출 안 함)·identify.ts·species.ts(시드 → 기기 캐시 → 서버). 사진 단계에서 넘어갈 때 인식을 시작하고, 한도 초과·장애·못 찾음은 모두 텍스트 검색으로 넘어간다(시뮬레이터에서 폴백 확인). 식물 상세에 관리 카드. 미구현: 부위 태그(사진은 leaf 로 보냄), Claude 비전 폴백(SPEC 9.1) |
