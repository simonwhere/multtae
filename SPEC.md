...
| 데이터 | 내보내기(JSON), 가져오기(합치기·모두 바꾸기), 전체 삭제. 가족과 나눠 쓸 때는 파일을 보내 받는 쪽이 합친다. 서버를 쓰지 않는다(9-2): 파일에만 있는 것은 더하고, 같은 식물은 물 준 상태를 더 최근에 물 준 쪽에서, 이름·자리 같은 나머지는 더 최근에 고친 쪽에서 가져온다. 설정은 기기마다 따로 두고, 지운 것은 전해지지 않는다 |
...
| 요소 | 내용 |
| --- | --- |
| 크기 | iOS 작게·중간(iOS 17 이상), 안드로이드 2×2 부터 늘릴 수 있음 |
| 내용 | 날짜와 요일, 오늘 물 줄 식물 수(밀린 것 포함), 이름 두세 줄(밀린 것이 먼저, 자줏빛과 "N일 지남" 글자), 넘치면 "외 N개". 없는 날은 "오늘은 물 줄 식물이 없어요"와 다음 예정 한 줄 |
| 갱신 | 앱이 알림을 다시 짤 때마다 오늘부터 7일치 문구와 색을 만들어 넘기고, 위젯은 오늘 날짜의 것을 고르기만 한다. 그래서 자정이 지나면 앱을 열지 않아도 넘어간다(iOS 는 자정마다 넘어가는 항목, 안드로이드는 한 시간마다 다시 그림). 7일을 넘기면 "앱을 열면 다시 맞춰요" |
| 누르면 | 오늘 탭 |
| 데이터 | 기기 안에서만 오간다(iOS App Group, 안드로이드 앱 문서 폴더). 서버를 쓰지 않는다 |

## 4. 등록 플로우

공간 등록 4단계, 식물 등록 6단계. 모든 단계는 뒤로 가기 가능하고 중간 이탈 시 임시 저장한다. 안드로이드 뒤로 가기 단추는 앞 단계로 가고, 첫 단계에서는 닫기와 같다. 키보드가 떠도 이전·다음 단추는 키보드 위에 보인다 (9-4).
...
비료 알림은 물주기 알림에 "비료도 함께" 한 줄로 덧붙이고, 분갈이 후 4주간은 자동 중단한다. 비료를 준 기록이 없으면 등록일부터 간격을 센다(새로 들인 식물에 바로 권하지 않는다). 물 줬어요 시트에서 비료 차례인 날에만 "비료도 줬어요"를 묻고, 고르면 fertilize 이벤트로 남긴다.
...
5. 안드로이드 (9-4): 권한 창을 띄운 적이 있는지 settings.notification_asked 에 적는다. 안드로이드 13 이상은 묻기 전에도 권한 상태를 거부로 알려 오기 때문이다(기기마다 다르므로 내보내지 않는다). 예약은 정확한 알람 권한 없이 걸어서 시스템이 정한 시각부터 최대 1시간 안에 묶어 울린다. 물주기처럼 분 단위가 중요하지 않은 알림에는 이쪽이 전지에 낫고, 정확한 알람 권한은 스토어 정책상 알람·달력 앱의 것이다. 상태 표시줄 아이콘은 잔가지 실루엣(assets/images/notification-icon.png, scripts/icons.py)에 accent 색이다.
...
한글은 낱말 단위로 줄을 바꾼다("위치 권 / 한"처럼 낱말 중간에서 끊지 않는다). iOS 는 AppText 의 lineBreakStrategyIOS, 안드로이드는 그런 설정이 없어 붙은 글자 사이에 보이지 않는 WORD JOINER 를 넣는다(src/lib/keep-words.ts, 앱 글자·시스템 확인 창·위젯 모두, 9-4). 안드로이드 시스템 창(확인 창 단추, 시각 고르기, 글자 커서)의 강조색도 accent 로 맞춘다.
...
| 9 | 출시 전 보강 | 9-1 종 데이터 검수(분재 주기·오분류·국명) / 9-2 가족과 나눠 쓰기(파일로 보내고 합치기, 서버를 쓰지 않는다) / 9-3 홈 화면 위젯(오늘 물 줄 식물, iOS·안드로이드) / 9-4 안드로이드 에뮬레이터 점검과 수정 | 경쟁 앱 대비 약점 보완 (2026-09-24 추가) |
...
| 4 | 애플워치 알림 (홈 위젯은 9-3 으로 앞당겼다) | 사용자 요청 시 |
