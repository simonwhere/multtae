...
| 계산 근거 공개 | 왜 이 날짜인지 숫자로 보여줌 | 식물 상세에 계수 곱셈식 표시, 사용자가 보정 가능 |
...
4. "다음 물주기: 9월 27일 (7일 × 가을 1.0 × 중형 1.0 × 중광 1.0)" 표시. 알림 시각 기본 오전 8시.
...
| 완료 | 별명 입력(기본값 국명), 첫 물주기 날짜와 계산식 표시, 마지막 물 준 날 입력(기본 오늘) | 식물 최대 200개 |
...
| "내일로" 미루기 | 다음 물주기 +1일, 최대 3회 연속, 이후 밀림으로 표시 |
...
| 하루 상한 | 하루 최대 4개 (물주기 1, 분재 2, 경고 1). 작업·분갈이는 물주기 알림에 한 줄로 합침 |
...
1. `rescheduleAll()`: 기존 예약 전부 취소 → plants 순회해 next_water_at 계산 → 날짜별로 그룹 → 14일치 예약.
...
콘셉트는 "마르는 흙"이다. 앱 전체가 흙이 젖었다가 마르는 과정을 색과 형태로 보여주고, 물 줄 때가 되면 화면이 말라 보인다. 초록 잎 아이콘과 물방울 일러스트로 가득한 기존 식물 앱과 반대로 간다.

### 14.1 핵심 시각 장치: 흙 게이지

| 요소 | 내용 |
| --- | --- |
| 형태 | 식물 카드 하단에 가로로 긴 흙 단면 띠. 왼쪽이 젖은 흙(진한 갈색), 오른쪽으로 갈수록 마른 흙(옅은 황토) |
| 동작 | 마지막 물 준 날부터 다음 물주기까지 진행률에 따라 젖은 영역이 줄어든다. 오늘이면 띠 전체가 마른 색이고 잔금 텍스처가 생긴다 |
| 분재 | 띠 대신 얕은 화분 단면, 마름 속도가 눈에 띄게 빠름 |
| 밀림 | 띠에 균열 패턴, 색은 회갈색 |
| 물 줬어요 | 띠가 왼쪽부터 어두워지는 0.6초 애니메이션, 물 번지는 느낌 |

### 14.2 색

| 토큰 | 라이트 | 다크 | 용도 |
| --- | --- | --- | --- |
| ink | #4A3728 | #EDE4D3 | 글자와 선(아이콘, 게이지의 금, 화분 윤곽), 주 버튼 면. 다크에서 soil.wet 은 배경과 대비가 2:1 이라 글자 전용으로 분리 |
| surface | #FBF8F3 | #28221C | 카드 면 (14.4의 paper보다 한 단계 밝은 면색) |
| soil.wet | #4A3728 | #5C4534 | 젖은 흙 |
| soil.dry | #C9B08A | #8A7355 | 마른 흙, 게이지 배경 |
| soil.crack | #9E8B76 | #6B5D4D | 밀림 |
| paper | #F4EFE6 | #1C1814 | 배경 (종이·화분 안쪽 느낌) |
| moss | #5B6B3E | #7A8C55 | 성공·완료·분재 뱃지 |
| water | #3F6E8C | #6F9EBB | 오늘 물주기 강조에만, 넓은 면 금지 |
| warn | #B5542E | #D4784F | 경고 카드 |

초록을 주색으로 쓰지 않는다. 초록은 완료 상태에만 쓴다.

### 14.3 타이포그래피

| 용도 | 서체 | 크기·굵기 |
| --- | --- | --- |
| 숫자·D-day | Instrument Serif (OFL) | 40~56pt Regular, 자간 −2% |
| 제목 | Pretendard | 20~24pt SemiBold |
| 본문 | Pretendard | 15pt Regular, 행간 1.55 |
| 학명 | Instrument Serif Italic | 13pt |
| 계산식 | Pretendard, tabular-nums | 13pt |

숫자를 세리프로 크게 쓰는 것이 이 앱의 인상을 결정한다. "D-3"이 화면의 주인공이다.

### 14.4 컴포넌트 원칙

| 원칙 | 내용 |
| --- | --- |
| 카드 | 모서리 12pt, 그림자 없음, paper보다 한 단계 밝은 면색과 1px 흙색 테두리 |
| 아이콘 | 선 1.5pt 단색, 식물 아이콘은 종이 아닌 식물군 6개만 구분(다육은 둥근 잎, 침엽은 바늘잎) |
| 사진 | 식물 사진은 원형이 아닌 3:4 세로 프레임 |
| 일러스트 | 온보딩·빈 상태에만, 갈색 단색 선화 |
| 모션 | 전환 200ms, 흙 게이지 600ms, 그 외 없음 |
| 문구 | 존댓말, 짧게, 느낌표 없음, 대시 문장부호 없음. "물때예요", "흙 확인할 때" |
| 이모지 | 앱 내 사용 금지 |
| 다크 모드 | 필수, 흙 색이 어두운 배경에서 더 자연스러움 |

### 14.5 화면별 인상

| 화면 | 인상 |
| --- | --- |
| 오늘 | 상단 큰 세리프로 오늘 날짜와 계절 모드, 그 아래 마른 게이지 카드가 먼저 |
| 식물 상세 | 세로 사진 옆에 D-day 대형 숫자, 아래 계산식이 영수증처럼 한 줄 |
| 공간 | 공간 사진 위에 방향 나침반과 빛 등급 4칸 게이지 오버레이 |
| 등록 | 한 화면 한 질문, 진행 표시는 흙 게이지가 차오르는 형태 |
...
| 2 | 등록과 알림 | 2-1 공간 등록 4단계(AI 없이 기본값 표) / 2-2 식물 등록 6단계(텍스트 검색, 번들 시드 30종) / 2-3 오늘 탭 + 물 줬어요 시트 + 학습 보정 / 2-4 로컬 알림 스케줄러 rescheduleAll / 2-5 식물 상세 + 계산식 표시 | 내 식물 등록하고 알림 받기 시작 |
...
- [ ] 스크린샷 6장 (6.7인치·6.5인치·5.5인치 iOS, 폰·태블릿 안드로이드): 오늘 탭, 공간 등록 AI 결과, 식물 상세 계산식, 분재 캘린더, 날씨 경고, 기록
